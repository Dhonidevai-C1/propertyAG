...
--- Indexes ---
CREATE INDEX idx_properties_agency ON properties(agency_id) WHERE is_deleted = FALSE;
CREATE INDEX idx_clients_agency ON clients(agency_id) WHERE is_deleted = FALSE;
...
CREATE POLICY "agency_update" ON agencies FOR UPDATE
  USING (id = my_agency_id() AND my_role() = 'admin');
...
CREATE POLICY "profiles_insert_admin" ON profiles FOR INSERT
  WITH CHECK (agency_id = my_agency_id() AND my_role() = 'admin');

-- properties: all team can read, agent/admin can write, only admin/creator can delete
...
CREATE POLICY "properties_delete" ON properties FOR UPDATE
  USING (agency_id = my_agency_id() AND my_role() = 'admin');
...
-- matches: all team can read, system inserts (via service role)
...
--- Supabase Storage ---
-- Storage bucket must be created manually in Dashboard: 'property-images' (Public: TRUE)
